...
//  RealHome
//
//  Created by boqian cheng on 2017-10-05.
...
class PropertyDetailsVC: UIViewController {
    ...
    public var bldgTypePassedIn: String?
    public var statePassedIn: String?
    ...
    @IBOutlet weak var associationCA: UILabel!
  ...
  //  let localSource = [AlamofireSource(urlString: "https://api.androidhive.info/json/movies/3.jpg")!]
    ...
    var aptDataCaptions: [String] = []
    var houseDataCaptions: [String] = []
    var plexDataCaptions: [String] = []
    var aptDataValues: [String?] = []
    var houseDataValues: [String?] = []
    var plexDataValues: [String?] = []
    
    var roomDataArr: [[String:String?]] = []
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        aptDataCaptions = LanguageProperty.getAptDataCaptions()
        plexDataCaptions = LanguageProperty.getPlexDataCaptions()
        houseDataCaptions = LanguageProperty.getHouseDataCaptions()
        
        self.descriptionCapLabel.text = LanguageProperty.generalDescriptionStr
        self.roomCapLabel.text = LanguageProperty.roomsStr
        ...
        self.roomTableViewHeight.constant = self.calculateTableHeight(tableView: self.roomTableView)
 ...
 /*
        let bldgType = "apt"
        if bldgType == "apt" {
            self.roomContainerHeight.constant = self.roomTableViewHeight.constant + 26
        } else {
            self.roomContainerView.clipsToBounds = true
            self.roomContainerHeight.constant = 0
        }
 */
    }
    
    fileprivate func setupPhotosSlider() {
        ...
        let recognizerPhoto = UITapGestureRecognizer(target: self, action: #selector(PropertyDetailsVC.didTapPhoto))
    ...
    fileprivate func setupPriceField() {
        ...
        let heartTap = UITapGestureRecognizer(target: self, action: #selector(PropertyDetailsVC.addFavoriteProp))
    ...
    fileprivate func broadNotify(resiCommIn: String) {
        ...
        noteCenter.post(name: NSNotification.Name(rawValue: "AddFavoriteFromDetailResi"), object: nil, userInfo: dictionaryInfo)
    }
    ...
    fileprivate func setupMapField() {
        ...
        let recognizerMap = UITapGestureRecognizer(target: self, action: #selector(PropertyDetailsVC.didTapMap))
    ...
    fileprivate func setupRoomTable() {
        
        self.descriptionConLabel.text = ""
        
        self.roomTableView.dataSource = self
        self.roomTableView.delegate = self
        self.roomTableView.isScrollEnabled = false
        self.roomTableView.estimatedRowHeight = 50.0
    ...
    fileprivate func setupBrokerageField() {
        ...
        self.emailBtn.addTarget(self, action: #selector(PropertyDetailsVC.toEmailToBrokerPage), for: .touchUpInside)
    }
    ...
    fileprivate func setupOtherFields() {
        ...
        self.schoolBtn.addTarget(self, action: #selector(PropertyDetailsVC.gotoSchoolSite), for: .touchUpInside)
        self.moreInfoBtn.addTarget(self, action: #selector(PropertyDetailsVC.gotoMoreInfo), for: .touchUpInside)
        
        let realtorTap = UITapGestureRecognizer(target: self, action: #selector(PropertyDetailsVC.gotoRealtorsSite))
        self.realtorCA.isUserInteractionEnabled = true
        self.realtorCA.addGestureRecognizer(realtorTap)
        
        let associateTap = UITapGestureRecognizer(target: self, action: #selector(PropertyDetailsVC.gotoAssociateSite))
        self.associationCA.isUserInteractionEnabled = true
        self.associationCA.addGestureRecognizer(associateTap)
    }
    ...
    fileprivate func getPropFromServer() {
        ...
        if let _bldgtype = self.bldgTypePassedIn, _bldgtype.count > 0 {
            if _bldgtype.lowercased().range(of: "apt") != nil || _bldgtype.lowercased().range(of: "apartment") != nil || _bldgtype.lowercased().range(of: "condo") != nil {
                parameterH = ["op": "q4",
                              "listingID": self.listingIDPassedIn ?? ""]
            } else if _bldgtype.lowercased().range(of: "duplex") != nil || _bldgtype.lowercased().range(of: "triplex") != nil || _bldgtype.lowercased().range(of: "fourplex") != nil {
                parameterH = ["op": "q6",
                              "listingID": self.listingIDPassedIn ?? ""]
            } else {
                parameterH = ["op": "q5",
        ...
        netWorkProvider.rx.request(.propSingleID(paras: parameterH)).subscribe { [unowned self] event in
            ...
            case .success(let response):
                do {
                    ...
                    debugPrint("Mapping Error in prop detai-resi: \(error.localizedDescription)")
                }
            case .error(let error):
                self.presentAlert(aTitle: nil, withMsg: LanguageGeneral.errMsgNetworkErrorStr, confirmTitle: LanguageGeneral.okStr)
                debugPrint("Networking Error in prop detail-resi: \(error.localizedDescription)")
    ...
    fileprivate func parseNetworkDataProps() {
        ...
        if let _bldgtype = self.bldgTypePassedIn, _bldgtype.count > 0 {
            if _bldgtype.lowercased().range(of: "apt") != nil || _bldgtype.lowercased().range(of: "apartment") != nil || _bldgtype.lowercased().range(of: "condo") != nil {
                ...
                let bldgtypeH = self.propertyObj["bldgtype"] as? String
                let bedsH = self.propertyObj["bedrooms"] as? String
                let bathH = self.propertyObj["bathrooms"] as? String
                let squareH = self.propertyObj["sqarefootage"] as? String
                let floorLh = self.propertyObj["floorlevel"] as? String
                let unitH = self.propertyObj["unitno"] as? String
                let propTaxH = self.propertyObj["propertytax"] as? String
                let mainFee = self.propertyObj["maintenanceFee"] as? String
                let parkType = self.propertyObj["parkingtype"] as? String
                let parkSpace = self.propertyObj["parkingspace"] as? String
                let heatType = self.propertyObj["heatingtype"] as? String
                let coolType = self.propertyObj["Coolingtype"] as? String
                let bldgAmen = self.propertyObj["bldgamenities"] as? String
                let propMgmt = self.propertyObj["propertymgmt"] as? String
                let brokageH = self.propertyObj["Brokerage"] as? String
                
                self.aptDataValues = [listIDh, listDateH, listDaysH, bldgtypeH, bedsH, bathH, squareH, floorLh, unitH, propTaxH, mainFee, parkType, parkSpace, heatType, coolType, bldgAmen, propMgmt, brokageH]
                self.generalTableView.reloadData()
                self.generalTableViewHeight.constant = self.calculateTableHeight(tableView: self.generalTableView)
            } else if _bldgtype.lowercased().range(of: "duplex") != nil || _bldgtype.lowercased().range(of: "triplex") != nil || _bldgtype.lowercased().range(of: "fourplex") != nil {
                let listIDh = self.propertyObj["ListingID"] as? String
                ...
                let bathH = self.propertyObj["bathrooms"] as? String
                let squareH = self.propertyObj["sqarefootage"] as? String
                let storeyH = self.propertyObj["storey"] as? String
                let ageH = self.propertyObj["age"] as? String
                ...
                let heatType = self.propertyObj["heatingtype"] as? String
                let coolType = self.propertyObj["Coolingtype"] as? String
                let basementH = self.propertyObj["basement"] as? String
                let swimH = self.propertyObj["swimmingpool"] as? String
                let brokageH = self.propertyObj["Brokerage"] as? String
                
                self.plexDataValues = [listIDh, listDateH, listDaysH, bldgtypeH, bedsH, bathH, squareH, storeyH, ageH, landSizeH, propTaxH, parkType, parkSpace, heatType, coolType, basementH, swimH, brokageH]
                self.generalTableView.reloadData()
                self.generalTableViewHeight.constant = self.calculateTableHeight(tableView: self.generalTableView)
            } else {
                let listIDh = self.propertyObj["ListingID"] as? String
                ...
                self.houseDataValues = [listIDh, listDateH, listDaysH, bldgtypeH, bedsH, bathH, squareH, storeyH, ageH, landSizeH, propTaxH, parkType, parkSpace, heatType, coolType, basementH, swimH, brokageH]
                self.generalTableView.reloadData()
                self.generalTableViewHeight.constant = self.calculateTableHeight(tableView: self.generalTableView)
            }
        } else {
        ...
        let roomsRet = self.propertyObj["rooms"] as? [[String:String?]]
        if let _roomsRet = roomsRet, _roomsRet.count > 0 {
            self.roomDataArr = _roomsRet
            self.roomTableView.reloadData()
            self.roomTableViewHeight.constant = self.calculateTableHeight(tableView: self.roomTableView)
        }
...
extension PropertyDetailsVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if tableView == self.generalTableView {
            if let _bldgtype = self.bldgTypePassedIn {
                if _bldgtype.lowercased().range(of: "apt") != nil || _bldgtype.lowercased().range(of: "apartment") != nil || _bldgtype.lowercased().range(of: "condo") != nil {
                    return aptDataCaptions.count
                } else if _bldgtype.lowercased().range(of: "duplex") != nil || _bldgtype.lowercased().range(of: "triplex") != nil || _bldgtype.lowercased().range(of: "fourplex") != nil {
                    return plexDataCaptions.count
                } else {
                    return houseDataCaptions.count
                }
            } else {
                return houseDataCaptions.count
            }
        } else if tableView == self.roomTableView {
            return roomDataArr.count
        } else {
            return 0
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView == self.generalTableView {
            let cell = tableView.dequeueReusableCell(withIdentifier: "generalTableCell", for: indexPath) as! GeneralTableCell
            if let _bldgtype = self.bldgTypePassedIn {
                if _bldgtype.lowercased().range(of: "apt") != nil || _bldgtype.lowercased().range(of: "apartment") != nil || _bldgtype.lowercased().range(of: "condo") != nil {
                    cell.titleLabel.text = self.aptDataCaptions[indexPath.row]
                    cell.contentLabel.text = self.aptDataValues[indexPath.row]
                } else if _bldgtype.lowercased().range(of: "duplex") != nil || _bldgtype.lowercased().range(of: "triplex") != nil || _bldgtype.lowercased().range(of: "fourplex") != nil {
                    cell.titleLabel.text = self.plexDataCaptions[indexPath.row]
                    cell.contentLabel.text = self.plexDataValues[indexPath.row]
                } else {
                    cell.titleLabel.text = self.houseDataCaptions[indexPath.row]
                    cell.contentLabel.text = self.houseDataValues[indexPath.row]
                }
            } else {
                cell.titleLabel.text = self.houseDataCaptions[indexPath.row]
                cell.contentLabel.text = self.houseDataValues[indexPath.row]
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "roomTableCell", for: indexPath) as! RoomTableCell
            
            let roomAti = self.roomDataArr[indexPath.row]
            
            if let roomNameStr = roomAti["name"] {
                cell.roomLabel.text = roomNameStr
            } else {
                cell.roomLabel.text = ""
            }
            if let roomSizeStr = roomAti["size"] {
                cell.sizeLabel.text = roomSizeStr
            } else {
                cell.sizeLabel.text = ""
            }
...
extension PropertyDetailsVC: MKMapViewDelegate {
