...
class MsgChatAgent: JSQMessagesViewController {
    
    public var buyerEmail: String?
    public var buyerNickname: String?
    ...
    var myAgentRef: DatabaseReference?
    var buyerRef: DatabaseReference?
    var buyerRefHandle: DatabaseHandle?
  ...
  //  var newMsgRef: DatabaseReference?
    ...
    override func viewDidLoad() {
        ...
        if let _agentGroupID = self.agentGroupID {
            ...
            if let _agentGroupRef = agentGroupRef {
                ...
                if let _messagesGroupRef = messagesGroupRef {
                    let emailCodedMy = FirebaseNodesCreation.decodeEmail(email: self.senderId)
                    myAgentRef = FirebaseNodesCreation.getSubNode(parentNode: _messagesGroupRef, node: emailCodedMy)
                    if let _myAgentRef = myAgentRef {
                        let buyerEE = self.buyerEmail ?? ""
                        let emailCodedBuyer = FirebaseNodesCreation.decodeEmail(email: buyerEE)
                        buyerRef = FirebaseNodesCreation.getSubNode(parentNode: _myAgentRef, node: emailCodedBuyer)
                        if buyerRef != nil {
                            observeMessages()
                        }
                    }
    ...
    private func observeMessages() {
        let messageQuery = buyerRef?.queryLimited(toLast: 1000)
        
        //   msgMyMsgRef?.queryOrdered(byChild: "sender").queryLimited(toLast: 25)
        
        // 2. We can use the observe method to listen for new
        // messages being written to the Firebase DB
        buyerRefHandle = messageQuery?.observe(.childAdded, with: { [unowned self] (snapshot) -> Void in
            // 3
            ...
            if let id = messageData["senderID"],
                ...
                print("Error! Could not decode message data in agent")
            }
        })
        ...
        updatedMessageRefHandle = buyerRef!.observe(.childChanged, with: { [unowned self] (snapshot) in
    ...
    {
      //  print("26-1048pmLiHome--" + self.photoMessageMap.count.description)
        
        let emailCodedMy = FirebaseNodesCreation.decodeEmail(email: self.senderId)
        let buyerEE = self.buyerEmail ?? ""
        let emailCodedBuyer = FirebaseNodesCreation.decodeEmail(email: buyerEE)
        let refPath: String = self.agentGroupID! + "/" + "messages" + "/" + emailCodedMy + "/" + emailCodedBuyer
    ...
    func setImageURL(_ url: String, forPhotoMessageWithKey key: String) {
        let itemImgRef = buyerRef!.child(key)
    ...
    */
    ...
    deinit {
        if let refHandle = buyerRefHandle {
            buyerRef?.removeObserver(withHandle: refHandle)
        }
...
extension MsgChatAgent: UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIPopoverPresentationControllerDelegate {
