...
protocol BedsRoomChangeSearch: class {
    func toChangeBedsRoom(_ cellView: BedsRoomTableCell, minRooms: Int, maxRooms: Int) -> Void
}

class BedsRoomTableCell: UITableViewCell, NHRangeSliderViewDelegate {

    @IBOutlet weak var bedsRoomLabel: UILabel!
    @IBOutlet weak var rangeSlider: NHRangeSliderView!
    
    fileprivate var lowVal: Double = 1
    fileprivate var upVal: Double = 3
    
    public weak var bedsRoomChoiceDelegate: BedsRoomChangeSearch?
    ...
    func sliderValueChanged(slider: NHRangeSlider?) {
        ...
        if let delegate = self.bedsRoomChoiceDelegate {
            delegate.toChangeBedsRoom(self, minRooms: Int(self.lowVal), maxRooms: Int(self.upVal))
        }
    }
